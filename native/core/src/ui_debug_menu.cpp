...
void UiDebugMenu::apply_action(){
 switch(UiDebugCategory(category_)){
 case UiDebugCategory::Teleport:if(cursor_==5){auto d=destination();auto f=debug_floor_at(context_,d,floor_);DebugTeleportRequest r;r.kind=d.kind;r.location=d.location;r.floor=f.error==Error::None?f.value:0;r.x=teleport_x_;r.y=teleport_y_;r.standard_entry=standard_entry_;last_teleport_request_=r;auto out=apply_debug_teleport(context_,r);last_teleport_status_=out.status;last_status_=out.status==DebugTeleportStatus::Applied?DebugStatus::Applied:DebugStatus::CoreRejected;has_result_=true;}break;
