...
Error restore_world_objects(const Json &state,QuestWorldServices &s){
    ...
    for(auto &v:arr.values){
        ...
        for(auto key:{"location","x","y"})if(!finite_int(v[key],0,255))return Error::NativeDomain;
        if(!finite_int(v["floor"],-1,255)||!finite_int(v["tile"],0,2047)||!finite_int(v["plotZ"],-1,255))return Error::NativeDomain;
        if(!finite_int(v["item"],0,8))return Error::NativeDomain;
        for(auto key:{"plot","shadowlord","search","loot","chest","prop","trapped","ship","torch"})if(v[key].kind!=Json::Bool)return Error::NativeDomain;
        if(!finite_int(v["itemId"],0,1023)||!finite_int(v["quality"],0,1023)||!finite_int(v["contents"],0,1023))return Error::NativeDomain;
        if(!finite_int(v["slot"],-1,255)||!finite_int(v["hull"],0,255)||!finite_int(v["skiffs"],0,255))return Error::NativeDomain;
        QuestObject o;
