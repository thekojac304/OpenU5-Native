...
const real=process.argv.includes('--real-arenas');
const directions=['east','west','south','north'] as const;
const realMaps:CombatMapData[]=real?JSON.parse(readFileSync(new URL('../../../game/assets/maps/combatmaps.json',import.meta.url),'utf8')):[];
if(real){
 deepStrictEqual(realMaps,parseCombatMaps(readFileSync(new URL('../../../original/u5/ultima5/BRIT.CBT',import.meta.url)),readFileSync(new URL('../../../original/u5/ultima5/DUNGEON.CBT',import.meta.url))));
 if(!check)mkdirSync(new URL('build-real-arenas/',root),{recursive:true});
}
...
if(!real)output('src/combat_tiles.inc',`// Generated from final TypeScript TILE_INFO / blocksSpellLine / combatMapForTile.\nstatic constexpr uint8_t kCombatTileFlags[] = {${flags.join(',')}};\nstatic constexpr uint8_t kCombatLos[] = {${los.join(',')}};\nstatic constexpr int8_t kCombatMapIndex[] = {${Array.from({length:TILE_INFO.length},(_,i)=>combatMapForTile(i)).join(',')}};\n`);
// Test-only transport of parsed JSON, not a second CBT decoder. Keep EA-derived data ignored.
if(real)output('build-real-arenas/maps.txt',realMaps.map(m=>[
 m.index,...m.tiles.flat(),...directions.flatMap(d=>[m.playerStarts[d].length,...m.playerStarts[d].flatMap(p=>[p.x,p.y])]),
 m.units.length,...m.units.flatMap(p=>[p.x,p.y]),m.triggers?.length??0,
 ...(m.triggers??[]).flatMap(t=>[t.sprite,t.at.x,t.at.y,t.pos1.x,t.pos1.y,t.pos2.x,t.pos2.y]),
].join(' ')).join('\n')+'\n');
...
for(let k=0;k<(real?128*4:78);k++)for(let si=0;si<(real?8:32);si++){
 const entryDirection=real?directions[k%4]!:'south';
 ...
 let map:CombatMapData={index:0,territory:'test',name:null,tiles:Array.from({length:11},()=>Array(11).fill(5)),playerStarts:{east:[],west:[],south:[{x:5,y:5},{x:4,y:5}],north:[]},units:[{sprite:0,x:6,y:5},{sprite:0,x:8,y:5},{sprite:0,x:5,y:7}],triggers:[]};
 ...
 if(real)map=structuredClone(realMaps[Math.floor(k/4)]!);
 ...
 const c:any=new Combat({map,entryDirection,party:s.characters.map((record:any,charIdx:number)=>({record,charIdx,weapons:characterWeapons(record,attack,range)})),enemies:[{def,count:k%8===0?0:k%13===12?16:3}],seed,state:s,defenseValues:defense,spellAttackRange:strength,roomCombat:real?map.territory==='dungeon':k%5===0});
 ...
 const ev=Game.prototype.startCombat.call(mock,{defIndex:def.index} as any,entryDirection,{intro:k%2?'attacked':'none'});
...
for(const key of real?['event:moved','event:died','event:ended','event:projectile','trigger:spent','text:Blocked!','text:VICTORY!','text:All must use the same exit!']:['ammo:depleted','ammo:wrapped','trigger:spent','event:moved','event:died','event:ended','event:projectile','attack:false:none','attack:true:0','attack:true:1','attack:true:20','attack:true:99','text:Blocked!','text:Out of range.','text:VICTORY!','text:All must use the same exit!','text:Thy sword hath shattered!','text:Rat interferes!'])if(!coverage[key])throw Error(`missing combat coverage ${key}`);
const destination=real?'build-real-arenas/':'fixtures/';
output(destination+'combat.txt',rows.join('\n')+'\n');output(destination+'combat-coverage.json',JSON.stringify({cases,coverage},null,2)+'\n');console.log(`${cases} ${real?'real-arena ':''}combat parity snapshots`);
