...
| D-24 | Outdoor (H)ole up on the device never asks "Wilt thou set a watch?" / "Who will stand guard?": every camp is unwatched, so no member is excluded from the partial heal, no guard walks, and the camp's random-number stream differs from a watched 1988 camp | incomplete | the prompt, `RestServices::guard_start` (the CampFire arena's south starts) and a `cell_free` that knows the guard's index were never built for the device; the TypeScript reference has all three | H-167 (supersedes H-160) |
| D-25 | An NPC killed by a ship's cannon keeps walking until the map is re-entered | incomplete | 1988 despawns it on the hit (`CMDS.OVL:0x0d47-0x0d82` → TOWN `0x0052` + `0x00b0`); **both ports** only set the dead bit | H-168 |
