...
| D-30 | Camp from a nonzero minute runs twelve five-minute steps per requested hour instead of stopping at the original target hour; e.g. 05:50 + one requested hour ends at 06:50 instead of 06:00 | incomplete, queued | CMDS `0x0066-0x0079` sets target hour and `0x01ee-0x01f8` tests it at every loop head; native `camp()` uses `hours` × 12 fixed steps. Distinct from H-171 per-step state/RNG and H-172 bed presentation; no Batch 36 production change | H-173 |
| D-31 | Original bed sleep refreshes the party status panel after `G→S` and per tick, but native presents only final roster state after the synchronous command returns; distinct from the map-window fill | queued, presentation | CMDS `0x060a` and `0x0674`; no Batch 37 correction | H-174 |
