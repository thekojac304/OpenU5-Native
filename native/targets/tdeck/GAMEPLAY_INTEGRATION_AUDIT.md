...
- Wiring In Ex Por's real door-unlock world effect (getdir prompt + `magic_door_open_worker`-equivalent tile mutation) natively, and the matching fix on the TypeScript side (`game/src/core/magic/cast.ts` case 26, `content-audit.md` PENDIENTE(3)) — both currently still implement the disproven "no-op" belief. Needs: a dedicated pre-flight guard (mirroring items 6/25 in `world_magic.cpp`), confirmation it must NOT share An Sanct's trapped-chest fallback, and a decision on In Ex Por's combat-context behavior (time-window mask `0x05` = town+combat, currently unmeasured).
