...
| **H-172** | Rest presentation | Original bed-entry viewport fill is absent | CMDS `0x0614-0x0624`: kernel `0x0a70` set color 0, `0x0aa6` fill rectangle (8,8)-(183,183), after NPC loop and `Zzzzzzz...`, before first tick | unadjudicated | generic renderer; no matching bed-entry fill | visual clearing during bed entry differs | low | any accepted bed sleep | **QUEUED, DISTINCT from H-169**: graphics calls follow the NPC loop and do not update its actors/marker | later presentation batch | no |
| **H-171** | Camp | The camp sleep loop omits original per-step kernel/world housekeeping | CMDS `0x0204` world tick (`0x5910`), `0x0207` ring sweep, `0x020a` housekeeping (`0x2900`), then `0x0314` clock +5 and `0x031b` housekeeping (`0x20fa`) | separate comparison pending | `camp_sleep_step` only advances five minutes and runs a ring sweep; it does not call these other kernels | poison, meals, timers and world/local progression can differ while camping | high | 100 % for missing calls | **QUEUED, UNFIXED — discovered during Batch 32 archaeology; distinct from H-167's watch choice/movement** | own batch; derive complete call semantics before implementation | yes |
...
**Separate queued finding.** Tracing the full camp loop exposed **H-171 / D-27**, not a reason to widen H-167. Original CMDS `0x0204` runs kernel `0x5910` world ticks, `0x020a` runs `0x2900` housekeeping, and `0x031b` runs `0x20fa` after the five-minute clock step; native `camp_sleep_step` currently calls only `advance_clock` and a ring sweep. Those missing world/survival effects are distinct from choosing and moving a watchman. H-171 is documented, unimplemented and untested as a fixed behavior. H-168, H-169 and H-170 were not changed.
