...
**Two RED assertions could not be reconciled and are left failing, not weakened:**
- **A8** (`blackthorn_machine_never_softlocks_the_world`) arms a `BlackthornSession` flag directly and calls `Pass` **without ever dispatching the resolving `BlackthornAction`**, then asserts the result is not `AwaitingResponse`. That assertion is the `commands.cpp:628` gate working *correctly* (the world should stay blocked while a prompt is genuinely unresolved) — A1/A3/A4/A5/A6, which exercise the same claim through the real end-to-end modal answer flow, already prove the flag clears and the softlock lifts. A8's own comment ("after X resolves … via a real modal answer flow") does not match its code (no resolution ever happens). Treated as a test-authoring defect, not a production one.
- **B5**'s third check (`native/core/tests/batch4_group_b_test.cpp`) asserts `!dialogue_session.active` after the same real `NpcInitiatesTalk` trigger B1 uses and requires to reach `BeginConversation`. Both tests forward the event through the identical `UiSession::consume()` path (`NpcWorld`'s default `ctx.events = ui.event_sink()` vs. B5's own tap, which the test's comment states forwards "exactly like `ui.event_sink()` would") — there is no mechanism available to the host-only `NpcWorld` fixture (no `AlphaRuntime`) that could make B1 pass while B5's third check also holds. Empirically confirmed: with the fix in place, `batch4_group_a` fails only its A8 sub-checks (4) and `batch4_group_b` fails only this one check.

**Evidence:** [STATIC] + [EXEC] (`batch4_group_a_tests`/`batch4_group_b_tests`, `quest_parity`).
...
**Post-Batch-4 addendum:** the authoritative suite grew from 61 to **63** registered tests (`batch4_group_a`, `batch4_group_b`). Result: **63 total, 60 pass, 3 fail**. `gameplay_parity` mismatch 2034 (R-21) is unchanged. `batch4_group_a` and `batch4_group_b` each carry one internal defect discovered, not introduced, by this batch's implementation — not a production regression:
- `batch4_group_a`'s **A8** (4 of its 32 checks) arms a `BlackthornSession` flag and calls `Pass` without ever dispatching the resolving `BlackthornAction`, then asserts the world is not `AwaitingResponse` — that is asserting the `commands.cpp:628` gate is broken when it is working as intended (A1/A3/A4/A5/A6 already prove the real end-to-end flow clears the flag and lifts the block).
- `batch4_group_b`'s **B5** (1 of its 12 checks) asserts `!dialogue_session.active` through the identical `UiSession::consume()` path that `B1` requires to dispatch `BeginConversation` and become `active` — the two cannot both hold in the host-only `NpcWorld` fixture (no `AlphaRuntime` layer exists there to make B1's requirement "deferred").

See R-09/R-10 above for the full analysis. Both are treated as test-authoring defects surfaced by this batch, not weakened or rewritten, per this batch's explicit instruction not to alter failing tests to force GREEN.
...
**Status (post-Batch-4):** R-09 and R-10 are **GREEN — RESOLVED**; see §3 for the full analysis, including the correction of the original R-09 write-up (severity was understated; the `CrystalBall`/`WellDrop` "correctly handled" claims were wrong and are now tracked as R-25/R-26, discovered but explicitly left unfixed). The authoritative `native/core/build-batch1-control` suite grew from 61 to **63** registered tests (`batch4_group_a`, `batch4_group_b`) and reports **63 total, 60 pass, 3 fail**: `gameplay_parity` mismatch 2034 (R-21, unchanged, pre-existing) plus one internal defect each in `batch4_group_a` (its A8 case, 4 checks) and `batch4_group_b` (one check in B5) — both are test-authoring defects surfaced by this batch's own analysis, not production regressions, and were left failing rather than weakened (see §7 addendum and §3 R-09 for the full reasoning). A pass mid-implementation briefly introduced a real regression — `quest_parity` mismatch 4821 — by placing the Blackthorn "Your response?" prompt suffix in the core event text instead of `UiSession`'s presentation layer; this was caught by the parity fixture itself, reverted, and re-implemented presentation-side; the final suite has **zero** `quest_parity` or other non-Batch-4/non-R-21 failures. T-Deck ESP-IDF 6.1 firmware was rebuilt and **passed**: `openu5_tdeck.bin` grew from `0xc63f0` to `0xc67c0` bytes (+976 bytes), app partition free space `23%` → `22%`, zero compiler warnings. Hardware flash was not performed at this validation point (deferred to the Batch 4 hardware checkpoint). See §3 R-09/R-10/R-25/R-26 and §14 Batch 4.
