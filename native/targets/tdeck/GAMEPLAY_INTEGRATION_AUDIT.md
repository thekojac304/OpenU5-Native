...
| `f` | Fire | **Y** | Reticle + Confirm path in `handle_modal`; `CellProjectile` has no renderer. Y-04 (still open -- Batch 7 resolved Quake only; see §14 Batch 7) |
...
| Poison tick feedback | **Y** | `PoisonTick` event unconsumed. Y-04 (open -- needs a new roster-row-invert renderer primitive; not touched by Batch 7, see §14 Batch 7) |
...
| Y-04 | Feedback VFX | `CellExplosion`, `CellProjectile`, `Refuge`, `TrollSneak`, `PoisonTick` still unconsumed. **`Quake` resolved (Batch 7)** -- see below | Five missing feedback channels remain: `CellExplosion`/`CellProjectile` need a temporary per-cell tile-override compositor (plus an ordering audit against the world-object mutation that already commits synchronously, per the reference's own `#243` write-up); `PoisonTick` needs a new party-roster row-invert renderer primitive that does not exist anywhere in native today; `Refuge`/`TrollSneak` each need their own modal scene pacer, architecturally the same class of work as the Blackthorn capture scene (§3 R-32, Batches 4.5B-4.5D) -- none of the three are a "consume the event" bolt-on, and rushing them risked exactly the "half-finished implementation" this audit's own instructions warn against. `MagicCeremony` **is** consumed and works, so the pattern exists for a plain timed non-blocking effect -- but neither `Refuge`/`TrollSneak` (modal, paced narrative) nor `PoisonTick` (needs a new display primitive) nor `CellExplosion`/`CellProjectile` (needs a new compositor path) are actually shaped like `MagicCeremony`; `Quake`'s square-wave shake was. |

#### Y-04 partial resolution (Quake) -- Batch 7
...
`Sfx` — Y-03. `PoisonTick`, `CellExplosion`, `CellProjectile`, `Refuge`, `TrollSneak` — Y-04, still open (Batch 7).
...
| Refuge / camp scenes | ✓ `RefugeScript` | event unconsumed | **Y-04** (open -- needs its own modal scene pacer, architecturally the same class of work as the Blackthorn capture scene (§3 R-32/Batches 4.5B-4.5D); not touched by Batch 7) |
...
**IDs:** R-12 (**GREEN — RESOLVED**), R-13 (**GREEN — RESOLVED**), Y-04 (**PARTIAL** -- Quake resolved; `CellExplosion`/`CellProjectile`/`PoisonTick`/`Refuge`/`TrollSneak` remain open)
...
### Batch 7 continued — Remaining Y-04 feedback channels · risk: **medium, mixed scope** · **NOT STARTED**
**IDs:** Y-04 (remainder): `CellExplosion`, `CellProjectile`, `PoisonTick`, `Refuge`, `TrollSneak`
**Work, per channel (none of these share one abstraction -- do not merge them just because one helper could technically touch all three shapes):**
- `CellExplosion`/`CellProjectile`: a temporary per-cell tile-override compositor in the presentation layer (host-testable shape: extend `PresentationSnapshot`/`compose_world_presentation` with a caller-supplied timed overlay list, mirroring how `reveal_all` was kept pure in Batch 7), plus resolving the ordering question the reference's own `#243` note raises (`underTile` must stay visible for the fx's `leadMs` before the state mutation that removes the sprite has already committed).
- `PoisonTick`: design and add a party-roster row-invert renderer primitive (native has none today, for the poison tick, the picker cursor, or combat damage flash -- the reference shares one primitive across all three; native has implemented none of them). This should be scoped and adjudicated on its own, since it touches shared party-display code no existing batch has designed yet.
- `Refuge`/`TrollSneak`: each needs a dedicated modal scene pacer. `native/core/include/openu5/blackthorn_scene.h`'s `BlackthornScenePacer` is the closest existing precedent but is deliberately Blackthorn-specific (its stage/slot model, room-patch format, and event-kind switch are hardcoded to that scene) -- generalizing it into a shared pacer is itself a design decision that needs its own adjudication pass, not an assumption made in passing by whichever batch happens to touch Refuge first.
**Physical test:** camp/refuge after a party wipe; get sneak-attacked by a troll; take poison damage while it ticks.
**Model:** Opus-level for the scene-pacer design questions (Refuge/TrollSneak, and whether to generalize `BlackthornScenePacer`); Sonnet for `PoisonTick`'s mechanical wiring once the roster-invert primitive is designed; Sonnet/Opus for `CellExplosion`/`CellProjectile` depending on what the `#243` ordering investigation finds.
