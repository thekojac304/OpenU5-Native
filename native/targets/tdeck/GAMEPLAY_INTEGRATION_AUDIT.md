...
| **H-175** | Camp apparition presentation | Original OUTSUBS processes each live member in roster order: heal and wake, standing actor, chime/flash/chord, conditional advancement message and getkey, MP recalculation and panel redraw; native commits all members before its cue loop and has no per-member acknowledgment | OUTSUBS 0x07fb-0x090e, especially 0x0792 and 0x07f5; original-byte Batch 40 log | older TypeScript Camp scene notes are secondary only | **QUEUED**: native rest.cpp and UiSession append events synchronously without a staged modal | Later member is already leveled at first member chime; original blocks between eligible members | medium | opt-in Batch 40 presentation RED probe 0/1 | **CONFIRMED SEPARATE PRESENTATION DEFECT**, no Batch 40 production edit | Phase 7B reserved after correction | yes |
