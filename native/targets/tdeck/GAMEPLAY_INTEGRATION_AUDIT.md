...
| Blackthorn / Falsehood / Abbey | **R** | R-09. Blackthorn's sacrifice roster compaction specifically is **G** (R-23 resolved, Batch 15); presentation/pacing (Y-31/Y-32) and the end-to-end ceremony are not covered by that. |
...
| Y-33 | Vas Rel Por (spell 46) never raises its phase-gate ceremony | **OPEN — pre-existing, restated by Batch 13, deliberately not fixed here.** `CAST.OVL 0x0cf0` prints "To phase:", reads the key, and only after the exact `'1'..'8'` gate (`0x0d1d`/`0x0d23`) pushes the literal index 8 (`0x0d2d`) into `CAST2:0x0000`; the three early exits skip it. The native command does not model the phase gate at all, so it raises no ceremony, and `check-gameplay.ts` excludes spell 46 explicitly rather than pretending to cover it. Noted by the Alpha-20 action-feedback pass at the time ("which the native command does not yet model") and untouched by Batch 13, whose scope was R-21. Needs the phase-key model first. |
