...
| **H-173** | Camp duration | A Camp begun at a nonzero minute runs too many five-minute steps | CMDS `0x0066-0x0079` stores `(start_hour + hours) mod 24`; loop head `0x01ee-0x01f8` exits as soon as `g_hour` equals the target. From 05:50, one hour stops at 06:00 after two steps | TypeScript comparison not an oracle | native `camp()` executes `hours` calls of twelve steps, so 05:50 reaches 06:50 | clock, torch/light minutes, redraw/wind/ring RNG and guard movement can overrun | med | original bytes and native loop are decisive; distinct from H-171 state/RNG effects per step | **QUEUED, UNFIXED — out of Batch 36 scope** | future Camp duration batch | yes |
