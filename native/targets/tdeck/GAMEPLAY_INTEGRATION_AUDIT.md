...
| **H-174** | Rest status presentation | Original status draw follows G-to-S and repeats after each bed tick's housekeeping; native omitted intermediate frames and sleeping active-selection clear | CMDS `0x060a`/`0x0674` -> EXE `0x2900`/`0x2726`, separate from viewport fill `0x0624` | confirmed | **HOST FIXED Batch 39**: synchronous status-only Board draw after state/housekeeping | S then tick HP/clock frames; black map persists | low | accepted bed sleep | **HOST FIXED** (17/17 framebuffer checks; nine mutations killed; 109/109 host) | Phase 7A TFT check | yes |
| **H-175** | Camp apparition staging | OUTSUBS 0x070b-0x0799 mutates and speaks for one eligible member, then waits at 0x0792; 0x079c-0x07f8 recalculates MP and draws status before the next slot | Batch 40 original-byte log and Batch 41 command/raw-key fixtures | 1988 original authoritative | **HOST FIXED Batch 41:** transient Camp continuation, one member per key, deferred RNG and status-only redraw | Intermediate roster/RNG now match original; save/menu shortcuts cannot interrupt | high | RED 0/1, focused 18/18, raw-key 9/9, nine killed mutations | **HOST FIXED; PHYSICAL 7B PENDING** | Phase 7B | yes |
| **H-176** | Camp apparition visual wake | OUTSUBS 0x0850-0x08c1 changes the standing actor tile and XOR flashes the viewport for each live member, with beeps and tones | Original OUTSUBS bytes and re/notes/camp-apparition-scene.md | TypeScript notes secondary | **QUEUED:** native emits chime/chord but has no Camp actor-wake tile or viewport XOR renderer path | Visual scene still lacks original sprite/flash, separately from corrected H-175 state/input staging | medium | original/native call-path comparison | **SEPARATE VISUAL DEFECT; UNFIXED IN BATCH 41** | Phase 7B retained | yes |
...
**Baseline.** Before production edits, HEAD was `270a31dae354e29f2c59babcd5b75fb11e91b269`, tag `alpha2-batch38-camp-duration` was annotated, and the tree was clean. The recorded Batch 38 full host suite was 108/108. The clean Batch 38 Launcher in `build-batch38` measured 869,856 bytes and direct SHA-256 `3c480336c85a8d4602ee0b3738793e523a027a40663778ed6fd87ea20b0d2804`. The generic `build/openu5_tdeck.bin` is an older unrelated build artifact.

**Original call and order.** `native/core/batch39-original-status-bytes.log` preserves shipped CMDS.OVL and ULTIMA.EXE hashes, bytes and 16-bit disassembly. After up to 16 pre-sleep NPC passes and the hostile cancellation gate, CMDS `0x05e6-0x0607` changes eligible party status G-to-S. `0x060a` calls the resident EXE `0x2900` status renderer; `0x060d` then prints `Zzzzzzz...`. The renderer selects the status text window, calls EXE `0x2726` for six party rows, redraws lower status fields, and restores the window. The rows present name, current HP and status letter. The row helper sets `g_active_char=0xff` if the selected member is S or D. Thus the entry frame shows S, pre-tick HP and clock, with that member deselected. Only after the message does CMDS `0x0614-0x0624` black-fill the map window. The first ten-minute advance is `0x0647`. Every tick calls `0x0671` housekeeping, `0x0674` status redraw, `0x0677` NPC snap and `0x0688` occupancy/ejection. The redraw includes any poison HP change and advanced clock, even on an ejecting tick, without repainting the map. The common wake epilogue changes S-to-G without a panel redraw: the final sleeping panel and black map persist until the next ordinary redraw.

**Native deficiency and correction.** Batch 38 assigned S in the right order but displayed the old panel until command return and retained the sleeping active selection. Bed entry now clears that selection and emits `BedStatusRefresh` before Zzz; each bed tick emits it after housekeeping and before NPC snap. AlphaRuntime sends the event synchronously to a narrow Board method. That method reuses normal six-row painting and updates the T-Deck's adjacent location/clock fields; it does not draw the map, frame or transcript. The original's status text window corresponds to this right status region. The existing black fill and sleep mechanics keep their established order. Item parity filters this presentation event without filtering semantic state.

**Proof.** New CTest `batch39_party_panel` drives the shipped-pack bed through raw Enter/H/1/Enter input. A framebuffer Board records S, HP, selection and clock at Zzz, each NPC snap and wake, along with map pixels and draw counts. The second, poisoned member loses HP from 100 to 94 across six ticks. Unchanged Batch 38 production was **9/17 GREEN, 8 RED** (`batch39-red.log`); corrected code is **17/17 GREEN** (`batch39-green.log`, `batch39-post-mutation.log`). Nine temporary mutations were killed and restored: omit entry (12/17), refresh before status (16/17), after Zzz (15/17), after blackout (14/17), omit tick (13/17), double tick (15/17), before housekeeping (14/17), retain selected sleeper (16/17), and full redraw (10/17). Raw mutation and matching build logs are retained under `native/core`.

**Regression and hardware.** The affected bed, Camp, renderer/UI, party/status and runtime subset passed **20/20** (`batch39-regression-ctest.log`). The fresh full host build passed **109/109**, one new CTest over Batch 38 (`batch39-final-build.log`, `batch39-final-ctest.log`). A clean ESP-IDF 6.1 firmware build hit an internal compiler error in unchanged ESP-IDF LCD code, then completed on a one-job retry (batch39-firmware-build.log and batch39-firmware-retry.log). The image is **870,640 bytes (0xd48f0)**, **+784 bytes** from Batch 38, with 0x2b710 bytes free in the 1 MiB app partition. The commit-stamped Launcher is packaged after commit; its SHA-256 is recorded in the annotated tag. The host Board captures planned framebuffer writes but cannot observe physical TFT/SPI text output. Phase **7A** is reserved for that check; no flash or physical validation occurred. Existing phases 6T-6Z are unchanged. No separate defect was found, and the advancement audit remains reserved for Batch 40.

## Batch 40 — advancement / level-up preservation audit

**Starting state.** HEAD cbb17eb04c6247bd93b4f837d19268dcee0588b8, annotated alpha2-batch39-party-panel-refresh, clean main branch ahead of origin/main by 31 commits. The existing Batch 39 host build was rerun at 109/109. Its image is 870,640 bytes; the tag records Launcher SHA-256 5797aba0495c0dbad2ac98095a5c112b476510612b6b1b1606824050f1731b95. Batch 39 production was not changed for this audit.

**Prior coverage.** The reverse-engineering notes re/notes/oracle-camp-event.md and re/notes/camp-apparition-scene.md already identified the Camp gate and much of the OUTSUBS body. The TypeScript Camp correction is useful history, not authority. Native rest.cpp already implemented the gate, level formula, HP set, stat draw, MP recalculation and message text; item parity includes a helper-level Camp sample, while the recent host Camp tests pin duration and gate timing. There was no dedicated original-backed native command-path advancement test.

**Original caller map and timing.** Kernel Camp entry 0x3c9a reaches CMDS.OVL Camp. An uninterrupted Camp completes its five-minute sleep loop, then CMDS 0x04e7 skips the event when its flags contain 0x82; otherwise 0x04f4 draws rand(0,99), and 0x04f7-0x0502 calls 0xbfd6 only for results 0..24. The relocated resident stub at 0x7f56 loads OUTSUBS and jumps to camp_results 0x0658. That routine alone contains the roster level-byte write at 0x070e. The direct disassembly and binary hashes are preserved in native/core/batch40-original-advancement.log; re/notes/oracle-camp-event.md records the resident live-RAM stub. Ordinary XP awards, ordinary turns, bed sleep, inn time passage, healers, shrines, Lord British and save/load do not call this advancement body. Camp is a trigger only when uninterrupted sleep reaches the apparition gate and the apparition wins. An interrupted ambush exits before the gate. XP is not evaluated on crossing a threshold.

**Eligibility, threshold and cap.** OUTSUBS 0x08e7-0x08f9 reads the member XP word, signed-divides by 100 and counts positive right shifts starting with level 1. For normal nonnegative XP, required values are L2=100, L3=200, L4=400, L5=800, L6=1600, L7=3200 and L8=6400. The equality at each threshold is eligible. At 0x0704 the computed level is compared for inequality with the stored level byte; the result is assigned directly, so one apparition can skip multiple levels and can reconcile an inconsistent stored level downward. There is no explicit level-8 clamp in this body. The original combat award caps normal earned XP at 9999, making L8 the effective normal maximum. Native clamps to 8 after the same calculation: equivalent for normal earned XP, a difference only for noncanonical XP above 9999.

**One member and many.** OUTSUBS 0x07fb loops party slots from zero to party_size minus one. Status D skips healing, wake visuals and level evaluation, but joins the class MP recalculation and panel refresh. Every other status, including poisoned or sleeping, is fully healed and changed to G before the eligibility check. For a changed level the original sets level, max HP and current HP to 30 times the computed level, then draws exactly one rand(1,3): 1 raises STR, 2 DEX, 3 INT, each capped at 30. The draw is consumed even when the selected stat is already capped. There is no random HP amount and no class or Avatar exception in this path. The class-only follow-up sets current MP to INT for A/M and INT divided by two for B, including dead members; other classes retain current MP. XP, food, equipment, karma and other roster fields are not changed by advancement itself. The apparition's separate full heal and status cure do change live members even without eligibility. Every eligible live member is processed in roster order, one draw each; ineligible and dead members consume no stat draw. No member's result changes another member's eligibility.

**Presentation and native verdict.** OUTSUBS prints An apparition!, plays materialization/arpeggio, fixes the apparition at the campfire, then wakes each live actor to a class tile with beep, chime, viewport XOR flash and chord. A changed member gets the Hail/name, computed level and stronger/quicker/wiser text, followed by getkey at 0x0792. The status panel redraw at 0x07f5 follows each member. After the loop, a KARMA.DAT speech indexed by karma is printed, acknowledged by getkey at 0x0961, and the old man vanishes. No level-up-specific music table was found beyond these scene tones. Native matches core state, gate and RNG behavior; the new default CTest is 21/21 through execute_command(Rest), covering 99/100/450 XP, multi-member order, dead skip, MP and capped stat, a missed gate, ordinary turn and 64 bed seeds. Eight mutations were killed. No production logic fix is warranted. The separate H-175/D-32 presentation defect is confirmed by the opt-in probe: at the first member's chime, a later member is already leveled (RED 0/1). Native also omits the per-member input hold and status redraw. That scene requires its own staged UI work and is queued, not absorbed into the progression audit.

**Validation and scope.** Focused 21/21; eight of eight mutations killed and source restored; affected combat/XP, party/Z-stats, Camp, bed, UI and persistence subset 15/15. The fresh host build passed and the complete suite is 110/110 versus Batch 39's 109/109 (native/core/batch40-final-build.log and batch40-final-ctest.log). Original disassembly, focused, mutation and RED presentation logs are retained under native/core. No progression production source changed, so no new firmware image or Launcher was built; Batch 39's 870,640-byte image and tag hash remain the current production baseline, with a zero-byte production delta. No hardware was flashed or tested. Existing phases 6T-6Z and 7A keep their assignments; Phase 7B is reserved for the future H-175 apparition staging check.

## Batch 41 — H-175 / D-32 staged Camp advancement

**Baseline and RED.** The tree began clean at Batch 40 commit 6edbb7de56b3f5d839d3471eab4552e6a18633bf, annotated alpha2-batch40-advancement-audit, main ahead of origin/main by 32 commits. The host suite was 110/110; the production app was 870,640 bytes and the Batch 39 Launcher SHA-256 was 5797aba0495c0dbad2ac98095a5c112b476510612b6b1b1606824050f1731b95. The unchanged native H-175 probe was RED 0/1: member 2 was already advanced at member 1's first chime (native/core/batch41-red-presentation.log).

**Original order.** The successful Camp apparition gate alone enters OUTSUBS. After “An apparition!”, materialization and arpeggio, it visits party slots in order. A dead slot skips cure, actor wake and XP evaluation, then recalculates class MP and redraws the status panel. A live slot is fully healed and changed to G, its standing actor wakes, and beep, chime, viewport XOR flash, chord and concluding beeps play. Then XP/100 determines the level. Only when it differs does the routine write level and both HP values as 30 times level, draw one rand(1,3), apply a capped STR/DEX/INT gain, and print the Hail/name cue beginning with Hail, [name]! and the following prose: “For thy valiant deeds, I shall reward thee! Thou art now level [n], and stronger!/quicker!/wiser!” The cue names the member and gives level and stat word; HP values appear through the panel; numeric stat values are not shown during the cue. OUTSUBS 0x0792 calls getkey_with_redraw without filtering: one key event, including direction or Escape, acknowledges the cue; no cancellation branch exists. A held key can advance again only if the source delivers a separate repeat event. MP recalculation and the status-only panel draw occur after the key and before the next slot or its RNG draw. An unchanged live member has wake/chime and MP/panel work but no stat draw, Hail or key wait. The XOR touches the viewport without a map redraw; no separate level-up music table was found. After the last slot, KARMA.DAT speech has its own getkey at 0x0961; the old man vanishes and Camp completes.

**Correction and intermediate proof.** Previously native mutated every member and consumed every stat draw before its first per-member cue, and omitted the member key waits and panel redraws. The real Rest command now keeps only a transient continuation phase and roster slot. It returns AwaitingResponse at each changed member's Hail and at the karma speech. CampAcknowledge finishes the prior member's MP and status-panel draw, then processes the next slot. The Batch 40 gate, level/HP/stat formulas, XP, party order and dead handling are unchanged. UiSession KeyWait consumes exactly one input event per continuation; raw device shortcuts are scene keys and cannot save/load or open a menu. Other gameplay commands are rejected during a wait. The original command loop cannot save during getkey, so no partial advancement state is persisted. Direct camp_wake helper fixtures retain their synchronous event shape; production Rest always stages.

The two-eligible fixture sees member 0 at L2/60 HP while member 2 is still L1, with one stat draw and no panel refresh. One directional key draws member 0 and the skipped dead slot, then stages member 2 at L3/90 HP with its deferred draw. A second key draws member 2 and the unchanged live slot, then waits on karma speech; the third completes Camp with no more draw. The second cue's RNG state is exactly one original draw after the first cue's state. The shipped-pack raw-key framebuffer fixture records one status-only draw per acknowledgement and no map redraw. Focused 18/18 and raw-key 9/9; nine targeted mutations killed: all-up-front, early RNG, dead processing, missing wait, missing panel, wrong slot, panel after next mutation, wrong first slot and ignored acknowledgement. Evidence: native/core/batch41-focused-green.log, batch41-raw-key-focused.log and batch41-mutation.log.

**Validation and scope.** The affected Camp, input, panel, item parity and persistence subset passed 16/16; the fresh full suite passed 112/112, two CTests above Batch 40. Logs: native/core/batch41-affected-ctest.log and batch41-full-ctest.log. The clean ESP-IDF 6.1 app is 871,424 bytes (0xd4c00), +784 from the 870,640-byte baseline, with 0x2b400 bytes free in the 1 MiB app partition (native/targets/tdeck/batch41-firmware-build.log). No hardware was flashed or tested. H-176 is a distinct queued visual gap: native still lacks the original standing actor tile and viewport XOR flash. Phase 7B remains reserved for the complete scene; phases 6T–6Z and 7A retain their assignments.
