...
### R-32 — Blackthorn capture/interrogation has no distinct scene presentation; the party visibly stays in the ordinary Palace lobby throughout · **SEVERITY 2 (fidelity, not a functional defect)** · **YELLOW — AUDITED, NOT FIXED (Batch 4.5C, scoped for a future 4.5D)**
...
**Proposed 4.5D scope (not implemented):**
- **Core (`native/core/`):** add a `GameEventKind::BlackthornScene` (or similar) carrying a small beat-list payload (figure placements, a handful of tile patches, pause markers) analogous to the TS reference's `CaptureSceneScript`/`CaptureSceneBeat`, emitted alongside the existing `Message`/`BlackthornPrompt` events at the same points `blackthorn.cpp` already transitions (capture entry, each interrogation round, `sacrifice()`, `deposit()`). `blackthorn.cpp`'s actual rule logic (mantra matching, roster mutation, position write) needs **no changes** — this is additive presentation metadata alongside it. The already-defined-but-unconsumed `RefugeScript`/`EndgameScript` beat-list types in `native/core/include/openu5/quest_world.h` are a directly reusable precedent for this shape (worth noting `RefugeScript` is itself currently emitted but never consumed by `ui_session.cpp`/`alpha_runtime.cpp` — a related, pre-existing dead path, out of scope here).
- **Device/runtime (`native/targets/tdeck/main/`):** a 4th presentation source in `AlphaRuntime::render()`'s dispatch (alongside `combat`/`dungeon3d`/`world`) that draws a small staged room from the beat payload — figures as simple sprite-tile draws, a couple of scripted tile patches (hourglass, pendulum/torture-table state, the sacrifice burst), and pacing gated the same way `UiSession`'s existing modal-advance-on-keypress pattern works. This is presentation/device work; no new core game rule.
- **Resource pack:** requires packing the throne-room scene asset (`shrine-scene.json`-equivalent) for the first time — a new packer entry in `native/tools/u5pack/alpha1.ts`, and a resource-pack version/size change (would need to be reported per this document's standing rule when it lands).
- **New tests:** a host-side scene-emission test (`blackthorn.cpp` emits the right beat sequence at each transition, byte-parity against the TS reference's beat builders where feasible) and a device smoke test that the new presentation source renders without touching the semantic state machine's existing coverage.
- **Scope discipline:** this is a substantial, multi-file batch (new event kind, new asset class, new render path) — explicitly **not** attempted in this pass, per this batch's instructions.

### R-33 — `searchObjects` basement entries compare the raw DATA.OVL floor byte (255) against the runtime floor convention (−1), so 5 basement search spots — including Gorn's brazier keys — can never be found · **SEVERITY 2** · **YELLOW — AUDITED, CLASSIFIED, NOT FIXED (Batch 4.5C)**
...
**Not fixed in this pass:** the fix touches shared reference logic (`search.ts`/`quest_search.cpp`) across 5 known entries and needs its own regression coverage in both engines — not a one-line, provably-safe change containable to this batch's scope, so it is reported here and left for a dedicated follow-up rather than risked inline.
